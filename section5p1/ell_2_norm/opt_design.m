...
tilde_Sigma = (0.02 * eye(nm)) + (0.002 * ones(nm,nm));
    ...
    cvx_begin sdp quiet
        ...
        subject to
            ...
            g_val + (grad_comp' * tilde_mu)...
                + (param.alpha * norm(grad_comp,2))...
                + trace(mat_W * tilde_Sigma)...
                + (param.beta * norm(mat_W,'fro'))...
...
% Output the results
% --->
fprintf('     uncertainty with L2-norm model \n');
...
for iSample = 1:param.num_data
    ...
    vec_z1 = param.alpha * (vec_z1 / norm(vec_z1,2));
    rng(rand_seed2(iSample),'twister');
    rand_mat = rand(nm,nm);
    rand_mat = (0.5*(rand_mat+rand_mat')) - 0.5;
    rand_mat = param.beta...
        * ( rand_mat / norm(rand_mat,'fro') );
    cvx_begin quiet
        cvx_solver sedumi
        cvx_precision best
    ...
    sample_opt_negative(iSample) =...
        ...
        + (param_kappa * norm(sqrtm(mat_Sigma) * grad_comp,2) )...
        - max_compliance;
